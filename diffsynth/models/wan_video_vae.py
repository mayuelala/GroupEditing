...
class WanVideoVAE(nn.Module):
    ...
    def encode(self, videos, device, tiled=False, tile_size=(34, 34), tile_stride=(18, 16), single_frame=False):
        videos = [video.to("cpu") for video in videos]
        # print("video size:", videos[0].shape)
        # video size: torch.Size([3, 9, 528, 528]) -> [channels, frame_count, height, width]
        ...
        for video in videos:
            if not single_frame:
                if tiled:
                    ...
                    hidden_state = self.tiled_encode(video.unsqueeze(0), device, tile_size, tile_stride) 
                else:
                    hidden_state = self.single_encode(video.unsqueeze(0), device)  
                hidden_states.append(hidden_state.squeeze(0))  
                # hidden_state size: torch.Size([16, 3, 66, 66])
            else:
                frame_hidden_states = []
                
                for i in range(video.shape[1]):  
                    frame = video[:, i, :, :].unsqueeze(1)
                    # print(f"Encoding frame {i+1}/{video.shape[1]} with shape {frame.shape}")
                    
                    if tiled:
                        tile_size = (tile_size[0] * self.upsampling_factor, tile_size[1] * self.upsampling_factor)
                        tile_stride = (tile_stride[0] * self.upsampling_factor, tile_stride[1] * self.upsampling_factor)
                        hidden_state = self.tiled_encode(frame.unsqueeze(0), device, tile_size, tile_stride)
                    else:
                        hidden_state = self.single_encode(frame.unsqueeze(0), device)
                    
                    frame_hidden_states.append(hidden_state.squeeze(0)) 
                    # print(f"Hidden state for frame {i+1} shape: {frame_hidden_states[-1].shape}")
                tmp_states = torch.cat(frame_hidden_states, dim=1)
                # print("tmp_states.shape:", tmp_states.shape)
                # tmp_states = torch.nn.functional.interpolate(tmp_states, size=((tmp_states.shape[2] + 3) // 4, tmp_states.shape[3], tmp_states.shape[4]), mode='nearest-exact').squeeze(0)
                hidden_states.append(tmp_states)
                # print("hidden_state size:", hidden_states[-1].shape)
    ...
    def decode(self, hidden_states, device, tiled=False, tile_size=(34, 34), tile_stride=(18, 16)):
        ...
        # hidden_states.shape: torch.Size([1, 16, 9, 66, 66])
        ...
        if not single_frame:
            ...
            for i in range(hidden_states.shape[2]):  
                state = hidden_states[:, :, i, :, :].unsqueeze(2)
                # print("state.shape:",state.shape)
                # state.shape: torch.Size([1, 16, 1, 66, 66])
                if tiled:
                    frame = self.tiled_decode(state, device, tile_size, tile_stride)
                else:
                    frame = self.single_decode(state, device)
                    # print(f"Encoding frame {i+1}/{video.shape[1]} with shape {frame.shape}")
            ...
            # print("video.shape:", video.shape)
            # video.shape: torch.Size([1, 3, 9, 528, 528])
